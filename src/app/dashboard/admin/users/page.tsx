                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-white/5 text-[10px] font-black uppercase text-gray-500 tracking-widest">
                                    ...
                                    <th className="p-6">Access Cluster</th>
                                    <th className="p-6">Subscription Tier</th>
                                    <th className="p-6">Network Load</th>
                                    <th className="p-6">Active Channels</th>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {users.map(u => (
                                    <React.Fragment key={u.id}>
                                        <tr key={u.id} className={`hover:bg-white/[0.02] transition-colors group cursor-pointer ${expandedUserId === u.id ? 'bg-white/[0.03]' : ''}`} onClick={() => setExpandedUserId(expandedUserId === u.id ? null : u.id)}>
                                            <td className="p-6">
                                            ...
                                            <td className="p-6">
                                                <code className="text-[10px] bg-white/5 px-2 py-1 rounded border border-white/10 text-primary">
                                                    {u.access_token || 'N/A'}
                                                </code>
                                            </td>
                                            <td className="p-6">
                                            ...
                                            <td className="p-6">
                                                <div className="text-xs space-y-1">
                                                    <div className="flex justify-between items-center gap-4">
                                                        ...
                                                        <span className="text-white font-bold">{u.messages_sent_current_period.toLocaleString()} <span className="text-gray-600 font-medium">/ {u.monthly_message_limit.toLocaleString()}</span></span>
                                                    </div>
                                                    <div className="flex justify-between items-center gap-4">
                                                        <span className="text-gray-500 uppercase font-black text-[9px] tracking-widest">Channels</span>
                                                        <span className="text-white font-bold">{u.instances?.length || 0} <span className="text-gray-600 font-medium">/ {u.max_instances}</span></span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-6">
                                                <div className="flex flex-wrap gap-1 max-w-[200px]">
                                                    {u.instances?.map((inst: any) => (
                                                        <span key={inst.id} title={inst.instance_id} className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded border ${inst.status === 'connected' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-red-500/10 text-red-500 border-red-500/20'}`}>
                                                            {inst.instance_id.slice(0, 4)}
                                                        </span>
                                                    )) || <span className="text-gray-600 font-medium text-[9px]">ZERO_NODES</span>}
                                                </div>
                                            </td>
                                            <td className="p-6">
                                                {u.is_active ?
                                        ...
                                        {expandedUserId === u.id && (
                                            <tr>
                                                <td colSpan={9} className="p-0 bg-white/[0.01]">
                                                    <motion.div
                                                            ...
                                                            {u.instances?.length > 0 ? u.instances.map((inst: any) => (
                                                                <div key={inst.id} className="bg-white/5 border border-white/5 rounded-2xl p-5 space-y-4">
                                                                    ...
                                                                    <div className="grid grid-cols-2 gap-4 pt-2 border-t border-white/5">
                                                                        <div>
                                                                            <div className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">Messages Sent</div>
                                                                        ...
                                                                        <div>
                                                                            <div className="text-[8px] font-black text-gray-500 uppercase tracking-widest mb-1">Phone Number</div>
                                                                            <div className="text-sm font-bold text-white">{inst.phone_number || 'UNKNOWN'}</div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            )) : (
                                                                <div className="col-span-full py-10 text-center text-gray-600 font-medium italic text-sm">
                                                                    No operational nodes registered for this identity.
                                                                </div>
                                                            )}
                                                        </div>
                                                    </motion.div>
            ...
            {editingUser && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0b0914]/80 backdrop-blur-xl p-6">
                    <motion.div
                        ...
                        className="carbon-card rounded-[2.5rem] shadow-2xl w-full max-w-2xl border-white/10 overflow-hidden relative"
                    >
                        ...
                        <div className="p-10 lg:p-12">
                            <div className="mb-10">
                                <div className="w-14 h-14 bg-primary/10 rounded-2xl flex items-center justify-center border border-primary/20 mb-6">
                                    <PencilSimple size={28} weight="bold" className="text-primary" />
                                </div>
                                <h3 className="text-3xl font-black mb-2">Override Node Config</h3>
                                <p className="text-gray-500 font-medium">Adjust parameters for operator: <span className="text-white">{editingUser.name}</span></p>
                            </div>
                            ...
                            <form onSubmit={handleSaveUser} className="space-y-6">
                                <div className="grid md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        ...
                                        >
                                            <option value="free" className="bg-carbon">FREE NODE</option>
                                            <option value="pro" className="bg-carbon">PRO NODE</option>
                                            <option value="enterprise" className="bg-carbon">ENTERPRISE CLUSTER</option>
                                        </select>
                                    </div>
                                ...
                                <div className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5">
                                    ...
                                    <span className="text-xs font-black uppercase tracking-widest text-gray-400">Node Status: <span className={editingUser.is_active ? 'text-green-500' : 'text-red-500'}>{editingUser.is_active ? 'Operational' : 'Suspended'}</span></span>
                                </div>
